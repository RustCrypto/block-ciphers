...
use cipher::{
    ...
    //consts::{U11, U13, U15, U16, U2, U24, U32},
...
//pub type Block128x2 = GenericArray<GenericArray<u8, U16>, U2>;
...
macro_rules! define_aes_impl {
    (
        ...
        pub struct $name {
            ...
            enc_keys2: [Bs8State<u32>; $rounds],
            dec_keys2: [Bs8State<u32>; $rounds],
        ...
        impl NewBlockCipher for $name {
            ...
            fn new(key: &GenericArray<u8, $key_size>) -> Self {
                ...
                let mut c =  Self {
                    ...
                    enc_keys2: [Bs8State(0, 0, 0, 0, 0, 0, 0, 0); $rounds],
                    dec_keys2: [Bs8State(0, 0, 0, 0, 0, 0, 0, 0); $rounds],
                    //enc_keys8: [k8; $rounds],
                    //dec_keys8: [k8; $rounds],
                };
                for i in 0..$rounds {
                    ...
                    c.enc_keys2[i] = bit_slice_fill_4x4_with_u32(
                        ek[i][0], ek[i][1], ek[i][2], ek[i][3],
                    );
                    c.dec_keys2[i] = bit_slice_fill_4x4_with_u32(
                        dk[i][0], dk[i][1], dk[i][2], dk[i][3],
                    );
...
*/
            ...
            fn encrypt_blocks(&self, blocks: &mut Block128x8) {
                ...
                for pos in (0..128).step_by(32) {
                    let blocks: &mut [u8] = &mut blocks[pos..(pos + 32)];
                    let bs = bit_slice_1x32_with_u32(blocks);
                    let bs2 = encrypt_core(&bs, &self.enc_keys2);
                    un_bit_slice_1x32_with_u32(bs2, blocks);
                }
            }
            ...
            fn decrypt_blocks(&self, blocks: &mut Block128x8) {
                ...
                for pos in (0..128).step_by(32) {
                    let blocks: &mut [u8] = &mut blocks[pos..(pos + 32)];
                    let bs = bit_slice_1x32_with_u32(blocks);
                    let bs2 = decrypt_core(&bs, &self.dec_keys2);
                    un_bit_slice_1x32_with_u32(bs2, blocks);
                }
            }
...
/*
            #[inline]
            fn encrypt_blocks(&self, blocks: &mut Block128x2) {
                #[allow(unsafe_code)]
                let blocks: &mut [u8; 16*2] = unsafe {
                    &mut *(blocks as *mut _ as *mut [u8; 32])
                };
                let bs = bit_slice_1x32_with_u32(blocks);
                let bs2 = encrypt_core(&bs, &self.enc_keys2);
                un_bit_slice_1x32_with_u32(bs2, blocks);
            ...
            #[inline]
            fn decrypt_blocks(&self, blocks: &mut Block128x2) {
                #[allow(unsafe_code)]
                let blocks: &mut [u8; 16*2] = unsafe {
                    &mut *(blocks as *mut _ as *mut [u8; 32])
                };
