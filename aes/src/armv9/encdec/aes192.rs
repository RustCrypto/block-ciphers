...
global_asm!{
    ...
    //      - x3     must be valid pointers to memory regions of at least 208 bytes
    //      - on exit: x1 and x3 remain unchanged
    //      - on exit: x0 is overwritten with cipher-data
    ".balign 8",
    ".arch armv8-a+sve-aes+sve2",
    ".global aes_armv9_encdec_aes192_encrypt",
    ".type aes_armv9_encdec_aes192_encrypt, %function",
    "aes_armv9_encdec_aes192_encrypt:",
        ...
        "ld1rqb {{z11.b}}, p0/z, [x3, #48]",    // broadcast load round 11 key
        "add x3, x3, #64",                      // key pointer += 64 bytes <4 keys>
    ...
    "1:",
        ...
        "aesmc z31.b, z31.b",
        "aese  z31.b, z31.b, z10.b",            // perform AES-128 round 10 encryption
        "aesmc z31.b, z31.b",
        "aese  z31.b, z31.b, z11.b",            // perform AES-128 round 11 encryption
        "eor   z31.b, z31.b, z12.b",            // perform AES-128 round 12 encryption
...
unsafe extern "C" {
    pub fn aes_armv9_encdec_aes192_encrypt(
        dst: *mut u8,
        src: *const u8,
        len: usize,
        key: *const u8,
...
global_asm! {
    ...
    ".arch armv8-a+sve-aes+sve2",
    ".global aes_armv9_encdec_aes192_decrypt",
    ".type aes_armv9_encdec_aes192_decrypt, %function",
    "aes_armv9_encdec_aes192_decrypt:",
        "mov x8, #0",                           // set loop counter {x8} to 0

        "whilelt p0.b, x8, x2",                 // set avl to len {x2} - loop counter {x8}
        "b.none 2f",                            // exit if avl == 0
        ...
        "ld1rqb {{ z2.b}}, p0/z, [x3, #32]",    // broadcast load round 02 key
        "ld1rqb {{ z3.b}}, p0/z, [x3, #48]",    // broadcast load round 03 key
        "add x3, x3, #64",                      // increment key pointer by 4 indices
        ...
        "ld1rqb {{z12.b}}, p0/z, [x3, #0 ]",    // broadcast load round 12 key
    "1:",
        "ld1b   z31.b, p0/z, [x1]",             // load avl bytes of cipher-data
        ...
        "aesimc z31.b, z31.b",
        "aesd   z31.b, z31.b, z10.b",           // perform AES-128 round 10 decryption
        "aesimc z31.b, z31.b",
        "aesd   z31.b, z31.b, z11.b",           // perform AES-128 round 11 decryption
        "eor    z31.b, z31.b, z12.b",           // perform AES-128 round 12 decryption

        "st1b   z31.b, p0, [x0]",               // save avl bytes of plain-data

        "incb x0",                              // increment plain-data pointer
        "incb x1",                              // increment cipher-data pointer
        "incb x8",                              // increment loop counter by avl indices
...
unsafe extern "C" {
    pub fn aes_armv9_encdec_aes192_decrypt(
        dst: *mut u8,
        src: *const u8,
        len: usize,
        key: *const u8,
...
#[inline(always)]
pub fn encrypt_vla(keys: &RoundKeys<13>, mut data: InOut<'_, '_, Block>, blocks: usize) {
    let dst = data.get_out().as_mut_ptr();
    let src = data.get_in().as_ptr();
    let len = blocks * 16;
    let key = keys.as_ptr().cast::<u8>();
    unsafe { aes_armv9_encdec_aes192_encrypt(dst, src, len, key) };
}
...
#[inline(always)]
pub fn decrypt_vla(keys: &RoundKeys<13>, mut data: InOut<'_, '_, Block>, blocks: usize) {
    let dst = data.get_out().as_mut_ptr();
    let src = data.get_in().as_ptr();
    let len = blocks * 16;
    let key = keys.as_ptr().cast::<u8>();
    unsafe { aes_armv9_encdec_aes192_decrypt(dst, src, len, key) };
}
