...
use cipher::{
    ...
    BlockCipher, BlockDecrypt, BlockEncrypt, KeyInit, BlockProcessing,
    InOutVal, InOutBuf, InResOutBuf,
};
...
impl BlockProcessing for Aes128 {
    type BlockSize = U16;
}
...
impl BlockEncrypt for Aes128 {
    fn encrypt_block(&self, mut block: impl InOutVal<Block>) {
        ...
        unsafe {
            aes128_encrypt1(&self.encrypt_keys, in_ptr, out_ptr);
        }
    }
...
impl BlockDecrypt for Aes128 {
    fn decrypt_block(&self, mut block: impl InOutVal<Block>) {
        ...
        let out_ptr = block.get_out() as *mut Block;
        unsafe {
            aes128_decrypt1(&self.decrypt_keys, in_ptr, out_ptr);
        }
    }
...
pub(crate) unsafe fn aes128_encrypt1(keys: &RoundKeys, in_ptr: *const Block, out_ptr: *mut Block) {
...
#[inline]
#[target_feature(enable = "aes")]
pub(crate) unsafe fn aes128_encrypt8(keys: &RoundKeys, in_ptr: *const Block8, out_ptr: *mut Block8) {
    ...
    aesenclast8(&mut blocks, keys[10]);
    store8(out_ptr, blocks);
}

#[inline]
#[target_feature(enable = "aes")]
unsafe fn aes128_decrypt1(keys: &RoundKeys, in_ptr: *const Block, out_ptr: *mut Block) {
    // Safety: `loadu` and `storeu` support unaligned access
    #[allow(clippy::cast_ptr_alignment)]
    let mut block = _mm_loadu_si128(in_ptr as *const __m128i);
...
#[inline]
#[target_feature(enable = "aes")]
pub(crate) unsafe fn aes128_decrypt8(keys: &RoundKeys, in_ptr: *const Block8, out_ptr: *mut Block8) {
    let mut blocks = load8(in_ptr);
    xor8(&mut blocks, keys[10]);
