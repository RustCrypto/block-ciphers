...
impl BlockProcessing for Aes256 {
    type BlockSize = U16;
}
...
impl BlockEncrypt for Aes256 {
    fn encrypt_block(&self, mut block: impl InOutVal<Block>) {
        ...
        unsafe {
            aes256_encrypt1(&self.encrypt_keys, in_ptr, out_ptr);
        }
    }
...
impl BlockDecrypt for Aes256 {
    fn decrypt_block(&self, mut block: impl InOutVal<Block>) {
        ...
        let out_ptr = block.get_out() as *mut Block;
        unsafe {
            aes256_decrypt1(&self.decrypt_keys, in_ptr, out_ptr);
        }
    }
    fn decrypt_blocks(
        &self,
...
pub(crate) unsafe fn aes256_encrypt1(keys: &RoundKeys, in_ptr: *const Block, out_ptr: *mut Block) {
...
#[inline]
#[target_feature(enable = "aes")]
pub(crate) unsafe fn aes256_encrypt8(keys: &RoundKeys, in_ptr: *const Block8, out_ptr: *mut Block8) {
    ...
    aesenc8(&mut blocks, keys[12]);
    aesenc8(&mut blocks, keys[13]);
    aesenclast8(&mut blocks, keys[14]);
    store8(out_ptr, blocks);
}

#[inline]
#[target_feature(enable = "aes")]
unsafe fn aes256_decrypt1(keys: &RoundKeys, in_ptr: *const Block, out_ptr: *mut Block) {
    // Safety: `loadu` and `storeu` support unaligned access
    #[allow(clippy::cast_ptr_alignment)]
    let mut block = _mm_loadu_si128(in_ptr as *const __m128i);
    block = _mm_xor_si128(block, keys[14]);
    block = _mm_aesdec_si128(block, keys[13]);
    block = _mm_aesdec_si128(block, keys[12]);
...
#[inline]
#[target_feature(enable = "aes")]
pub(crate) unsafe fn aes256_decrypt8(keys: &RoundKeys, in_ptr: *const Block8, out_ptr: *mut Block8) {
    let mut blocks = load8(in_ptr);
    xor8(&mut blocks, keys[14]);
    aesdec8(&mut blocks, keys[13]);
    aesdec8(&mut blocks, keys[12]);
