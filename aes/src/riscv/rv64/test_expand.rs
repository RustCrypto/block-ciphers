use crate::riscv::rv64::{RoundKeys, expand::*};
use hex_literal::hex;
...
fn load_expanded_keys<const N: usize>(blocks: [[u8; 16]; N]) -> RoundKeys<N> {
    blocks.map(|block| {
        let (block, tail) = block.as_chunks::<8>();
        assert!(tail.is_empty());
        core::array::from_fn(|i| u64::from_ne_bytes(block[i]))
    })
}

fn store_expanded_keys<const N: usize>(keys: RoundKeys<N>) -> [[u8; 16]; N] {
    keys.map(|key| {
        let [b0, b1] = key.map(u64::to_ne_bytes);
...
fn aes128_key_expansion() {
    let ek = unsafe { aes128_expand_key(&AES128_KEY) };
...
fn aes192_key_expansion() {
    let ek = unsafe { aes192_expand_key(&AES192_KEY) };
...
fn aes256_key_expansion() {
    let ek = unsafe { aes256_expand_key(&AES256_KEY) };
