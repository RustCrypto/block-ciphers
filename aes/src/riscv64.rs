//! AES block cipher implementation using the RISC-V Cryptography Extensions.
//!
...
//!
//!     Currently only the RISC-V scalar extensions are utilized in this implementation.
//!
//! For reference, see the following other implementations:
//!
//!     1. The RISC-V Cryptography Extensions "benchmarks" reference for RISC-V 64 with zkn{de}:
//!     https://github.com/riscv/riscv-crypto/tree/main/benchmarks/aes/zscrypto_rv64
//!
//!     2. The OpenSSL assembly implementation for RISC-V 64 with zkn{de}:
...
use self::{
    ...
    expand::{inv_expanded_keys, KeyScheduleState},
};
...
macro_rules! define_aes_impl {
    (
        ...
        impl KeyInit for $name_enc {
            ...
            fn new(key: &Key<Self>) -> Self {
                Self {
                    round_keys: KeyScheduleState::<$words, $rounds>::expand_key(key.as_ref()),
                }
            }
        }
