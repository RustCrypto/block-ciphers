...
mod utils {
    ...
    impl CipherState1 {
        ...
        pub(super) fn enc1_two_more(&mut self, k0: RoundKey, k1: RoundKey) {
            self.data[0] ^= k0[0];
            self.data[1] ^= k0[1];
            let mut n0 = unsafe { aes64esm(self.data[0], self.data[1]) };
            let mut n1 = unsafe { aes64esm(self.data[1], self.data[0]) };
            n0 ^= k1[0];
            n1 ^= k1[1];
        ...
        pub(super) fn dec1_two_more(&mut self, k0: RoundKey, k1: RoundKey) {
            let mut n0 = unsafe { aes64dsm(self.data[0], self.data[1]) };
            let mut n1 = unsafe { aes64dsm(self.data[1], self.data[0]) };
            self.data[0] = n0 ^ k1[0];
            self.data[1] = n1 ^ k1[1];
