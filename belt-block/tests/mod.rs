...
fn belt_wblock() {
    ...
    let y2 = hex!(
        "F08EF22D CAA06C81 FB127219 74221CA7 AB82C628 56FCF2F9 FCA006E0 19A28F16"
    ...
    let x3 = hex!(
        "92632EE0 C21AD9E0 9A39343E 5C07DAA4 889B03F2 E6847EB1 52EC99F7 A4D9F154"
        "B5EF68D8 E4A39E56 7153DE13 D72254EE"
    );
    let x4 = hex!(
        "DF3F8822 30BAAFFC 92F05660 32117231 0E3CB218 2681EF43 102E6717 5E177BD7"
        "5E93E4E8"
    );
    let y4 = hex!(
        "E12BDC1A E28257EC 703FCCF0 95EE8DF1 C1AB7638 9FE678CA F7C6F860 D5BB9C4F"
    ...
    let belt1 = BeltBlock::new_from_slice(&k1).unwrap();
    let belt2 = BeltBlock::new_from_slice(&k2).unwrap();

    let mut t1 = x1;
    belt1.wblock_enc(&mut t1);
    assert_eq!(t1, y1);
    belt1.wblock_dec(&mut t1);
    assert_eq!(t1, x1);

    let mut t2 = x2;
    belt1.wblock_enc(&mut t2);
    assert_eq!(t2, y2);
    belt1.wblock_dec(&mut t2);
    assert_eq!(t2, x2);

    let mut t3 = x3;
    belt2.wblock_enc(&mut t3);
    assert_eq!(t3, y3);
    belt2.wblock_dec(&mut t3);
    assert_eq!(t3, x3);

    let mut t4 = x4;
    belt2.wblock_enc(&mut t4);
    assert_eq!(t4, y4);
    belt2.wblock_dec(&mut t4);
    assert_eq!(t4, x4);
}
