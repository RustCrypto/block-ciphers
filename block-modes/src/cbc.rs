        ...
        -> Result<(), BlockModeError>
    {
        ...
        if pb != 1 {
            ...
            while buffer.len() >= bss {
                ...
                let mut blocks_copy = {
                    let ga_blocks = unsafe { &mut *(blocks.as_mut_ptr()
                ...
                let blocks_copy = unsafe {slice::from_raw_parts(
                    blocks_copy.as_mut_ptr() as *mut u8, bss - bs
                )};
        ...
        while buffer.len() >= bs {
            ...
            self.cipher.decrypt_block(unsafe { &mut *(block.as_mut_ptr()
