...
pub(crate) fn sm4_init_key<B: KeyInit>(key: &Key<B>) -> [u32; 32] {
    let mk = [
...
pub(crate) fn sm4_encrypt<B: BlockSizeUser>(mut block: InOut<'_, '_, Block<B>>, rk: &[u32; 32]) {
    let b = block.get_in();
    let mut x = [
...
impl BlockCipherEncBackend for Sm4 {
    ...
    fn encrypt_block(&self, block: InOut<'_, '_, Block<Self>>) {
        sm4_encrypt::<Self>(block, &self.rk)
    }
}
...
pub(crate) fn sm4_decrypt<B: BlockSizeUser>(mut block: InOut<'_, '_, Block<B>>, rk: &[u32; 32]) {
    let b = block.get_in();
    let mut x = [
        u32::from_be_bytes(b[0..4].try_into().unwrap()),
        u32::from_be_bytes(b[4..8].try_into().unwrap()),
...
impl BlockCipherDecBackend for Sm4 {
    ...
    fn decrypt_block(&self, block: InOut<'_, '_, Block<Self>>) {
        sm4_decrypt::<Self>(block, &self.rk)
    }
}
