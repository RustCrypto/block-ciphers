//! SM4 with AVX2
//!
//! Implementation was borrowed from <https://www.cnblogs.com/kentle/p/15562530.html> by kentle.
...
#[inline]
unsafe fn mm256_pack0_epi32(a: __m256i, b: __m256i, c: __m256i, d: __m256i) -> __m256i {
    _mm256_unpacklo_epi64(_mm256_unpacklo_epi32(a, b), _mm256_unpacklo_epi32(c, d))
}

#[inline]
unsafe fn mm256_pack1_epi32(a: __m256i, b: __m256i, c: __m256i, d: __m256i) -> __m256i {
    _mm256_unpackhi_epi64(_mm256_unpacklo_epi32(a, b), _mm256_unpacklo_epi32(c, d))
}

#[inline]
unsafe fn mm256_pack2_epi32(a: __m256i, b: __m256i, c: __m256i, d: __m256i) -> __m256i {
    _mm256_unpacklo_epi64(_mm256_unpackhi_epi32(a, b), _mm256_unpackhi_epi32(c, d))
}

#[inline]
unsafe fn mm256_pack3_epi32(a: __m256i, b: __m256i, c: __m256i, d: __m256i) -> __m256i {
    _mm256_unpackhi_epi64(_mm256_unpackhi_epi32(a, b), _mm256_unpackhi_epi32(c, d))
}

#[inline]
#[target_feature(enable = "avx2")]
pub(super) unsafe fn sm4_process8<T: ParBlocksSizeUser>(
    block: InOut<'_, '_, ParBlocks<T>>,
    rk: &[u32; 32],
    encrypt: bool,
) {
    ...
    let in_block2_ptr = in_ptr as *const __m256i;

    let mut b: [__m256i; 4] = [
        _mm256_loadu_si256(in_block2_ptr.add(0)),
        _mm256_loadu_si256(in_block2_ptr.add(1)),
        _mm256_loadu_si256(in_block2_ptr.add(2)),
        _mm256_loadu_si256(in_block2_ptr.add(3)),
    ];

    let mut x: [__m256i; 4] = [
        mm256_pack0_epi32(b[0], b[1], b[2], b[3]),
        mm256_pack1_epi32(b[0], b[1], b[2], b[3]),
        mm256_pack2_epi32(b[0], b[1], b[2], b[3]),
        mm256_pack3_epi32(b[0], b[1], b[2], b[3]),
    ];

    let vindex = _mm256_setr_epi8(
    ...
    for i in 0..32 {
        let k = if encrypt {
            ...
            _mm256_set1_epi32(rk[31 - i] as i32)
        };

        b[0] = _mm256_xor_si256(_mm256_xor_si256(x[1], x[2]), _mm256_xor_si256(x[3], k));

        b[1] = _mm256_xor_si256(
            x[0],
            _mm256_i32gather_epi32::<4>(BOX0.as_ptr() as *const _, _mm256_and_si256(b[0], mask)),
        );
        b[0] = _mm256_srli_epi32(b[0], 8);
        b[1] = _mm256_xor_si256(
            b[1],
            _mm256_i32gather_epi32::<4>(BOX1.as_ptr() as *const _, _mm256_and_si256(b[0], mask)),
        );
        b[0] = _mm256_srli_epi32(b[0], 8);
        b[1] = _mm256_xor_si256(
            b[1],
            _mm256_i32gather_epi32::<4>(BOX2.as_ptr() as *const _, _mm256_and_si256(b[0], mask)),
        );
        b[0] = _mm256_srli_epi32(b[0], 8);
        b[1] = _mm256_xor_si256(
            b[1],
            _mm256_i32gather_epi32::<4>(BOX3.as_ptr() as *const _, _mm256_and_si256(b[0], mask)),
        );
    ...
    _mm256_storeu_si256(
        out_block2_ptr.add(0),
        mm256_pack0_epi32(x[3], x[2], x[1], x[0]),
    );
    _mm256_storeu_si256(
        out_block2_ptr.add(1),
        mm256_pack1_epi32(x[3], x[2], x[1], x[0]),
    );
    _mm256_storeu_si256(
        out_block2_ptr.add(2),
        mm256_pack2_epi32(x[3], x[2], x[1], x[0]),
    );
    _mm256_storeu_si256(
        out_block2_ptr.add(3),
        mm256_pack3_epi32(x[3], x[2], x[1], x[0]),
    );
}

#[inline]
pub fn sm4_encrypt8<T: ParBlocksSizeUser>(blocks: InOut<'_, '_, ParBlocks<T>>, rk: &[u32; 32]) {
    unsafe { sm4_process8::<T>(blocks, rk, true) }
}

#[inline]
pub fn sm4_decrypt8<T: ParBlocksSizeUser>(blocks: InOut<'_, '_, ParBlocks<T>>, rk: &[u32; 32]) {
    unsafe { sm4_process8::<T>(blocks, rk, false) }
}
...
impl<'a> BlockCipherEncBackend for Sm4Enc<'a> {
    ...
    fn encrypt_par_blocks(&self, blocks: InOut<'_, '_, ParBlocks<Self>>) {
        sm4_encrypt8::<Self>(blocks, &self.0.rk);
    }
}
...
impl<'a> BlockCipherDecBackend for Sm4Dec<'a> {
    ...
    fn decrypt_par_blocks(&self, blocks: InOut<'_, '_, ParBlocks<Self>>) {
        sm4_decrypt8::<Self>(blocks, &self.0.rk);
    }
}
